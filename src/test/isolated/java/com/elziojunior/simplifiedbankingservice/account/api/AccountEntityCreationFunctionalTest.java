...
class AccountEntityCreationFunctionalTest {
    ...
    void shouldCreateAccountWithoutAuthenticationOrCsrfToken() throws Exception {
        when(accountService.createAccount(any(CreateAccountDto.class))).thenReturn(new CreatedAccountDto(
                41L,
                "Ada Lovelace",
        ...
        verify(accountService).createAccount(new CreateAccountDto("Ada Lovelace", new BigDecimal("100")));
    }
    ...
    void shouldTranslateApplicationValidationToProblemDetails() throws Exception {
        when(accountService.createAccount(any())).thenThrow(
